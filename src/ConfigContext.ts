...
export function updateConfigContext(newContext: Partial<ConfigContext>): void {
  ...
  if (!validateEmulatorEndpoint(newContext.EMULATOR_ENDPOINT)) {
...
// Note: EMULATOR_ENDPOINT is not set here to allow config.json to specify the port dynamically
// for Linux emulator Docker containers that may run on non-default ports.
